...
#ifdef	QIHOO_1603
extern Addr qihoo_addr; extern UInt qihoo_len;
#endif

extern Bool BG_(is_trace_syscall);
extern Bool BG_(is_release_dex_files);
...
void BG_(syscall_lseek)(ThreadId tid, UWord* args, UInt nArgs,
	...
	if (BG_(is_trace_syscall)) {
		BG_LOGI("SYSCALL(%d) lseek(%d) offset:0x%x, whence:0x%s, ret:0x%s, read_offset:0x%s\n", 
				tid, fd, (UInt)offset, whence, retval, fds[tid][fd].offset);
	}
...
void BG_(syscall_mmap)( ThreadId tid, UWord* args, UInt nArgs, SysRes res) {
	...
	if( begin_addr <= 0 || prot == PROT_NONE )
		return;
	if (BG_(is_trace_syscall)) {
		BG_LOGI("SYSCALL(%d) mmap(%d) off_0x%08x -> 0x%08x-0x%08x %8d %c%c%c 0x%x %s\n", 
				tid, fd, offset, begin_addr, begin_addr+size, size, 
				(prot & PROT_READ)  ? 'r' : '-',
				(prot & PROT_WRITE) ? 'w' : '-',
				(prot & PROT_EXEC)  ? 'x' : '-',
				flags,
				(fd > 0) ? fds[tid][fd].name : "Anonymous");
	}
	if ( isThirdFd(tid, fd) ) {
		if(fds[tid][fd].type == FdAppDex) {
		...
		if(prot & PROT_EXEC)/* Executable */
			addMemMap(begin_addr, size, prot, fds[tid][fd].name);
	}
	if (BG_(clo_trace_begin) == False)
		return;
	if(prot & PROT_EXEC) { /* Executable */
		if(fd <= 0) {
			addMemMap(begin_addr, size, prot, "anonymous.memory.map");
#ifdef	QIHOO_1603
			if (size == qihoo_len)
				qihoo_addr = begin_addr;
#endif
		}
	}
...
void BG_(syscall_mprotect) ( ThreadId tid, UWord* args, UInt nArgs, SysRes res) {
	...
	//return;
	if(prot & PROT_EXEC) { /* Executable */
		addMemMap(begin_addr, size, prot, "mprotect.to.executable");
	} else {
		delMemMap(begin_addr, size);
	}
}
...
void BG_(syscall_ptrace_pre)( ThreadId tid, UWord* args, UInt nArgs) {
...
void BG_(syscall_exit)( ThreadId tid, UWord* args, UInt nArgs) {
	Int status = (Int)args[0];
	if (BG_(is_trace_syscall)) {
		BG_LOGI("SYSCALL(%d) exit() status=%d\n", status);
	}
}
...
void BG_(syscall_write)(ThreadId tid, UWord* args, UInt nArgs, SysRes res) {
	...
	if (BG_(is_trace_syscall)) {
		BG_LOGI("SYSCALL(%d) write(%d) 0x%x %d %s\n", 
				tid, fd, (Int)data, curr_len, fds[tid][fd].name);
	}
}
