#ifndef _BG_OAT_DEX_PARSE_H
#define _BG_OAT_DEX_PARSE_H

#include "bevgrind.h"
#include "bg_oatplus.h"
...
/****************** dex file reassemble related structures *****************/

/*
 * the type of encoded value for encoded annotation items or encoded array
 */
typedef enum VALUE {
    VALUE_BYTE = 0x00,
    VALUE_SHORT = 0x02,
...
/*
 * The structure indicates what the classDef items should refer to
 */
struct ClassDefRefer {
    Int refer_typeList;
    Int refer_clzDataItem;
    Int refer_annoDir;
    Int refer_encodeArray;
};

/*
 * The structure indicates what the method code of class data items should refer
 * to
 */
struct MethodCodeRefer {
    Int* refer_code;
    u4 refer_code_num;
};
...
/*
 * Each item of dex data structure
 */
struct DexDataItem {
    u4 write_off;     // the offset to write its data
    u4 read_off;      // the offset to read its data
    UInt length;       // the length of the item
    void* data;       // exclusive to read_off, the addr of the data
    Int refer;        // used for IDDS to refer to CDDS collected   
        
    //void* refer_data; // it may be any above refer structure

    // move method code refer to item
    Int refer_code[200];
    u4 refer_code_num;

    // move class def refer to item 
    Int refer_typeList;
    Int refer_clzDataItem;
    Int refer_annoDir;
    Int refer_encodeArray;
...
/*
 * The set to store each kind of DDS which has many items
 */
#define MAX_ITEM_SIZE		64000
struct DexDataSet {
    struct DexDataItem items[MAX_ITEM_SIZE];    // the array to store its items
    u4 size;                     // the number of items
    u4 write_off;                // the start write offset of all the items
    Bool isWordAlign;            // the write of these items need word-align ?
};

/*
 * The memory allocated to write the dex file data collected
 */
struct MemMap {
    u1* startAddr;   // the start addr of the memory
    u1* dexAddr;     // the start addr of the dex file data
    u4 length;       // the length of the memory
    u4 dexLength;    // the length of the dex file data written
};

/*
 * The overall structure of DexFile data (refer to dalvik dex format)
 */
struct DexFileData {
    struct DexDataItem* header;
    struct DexDataSet* stringIds;
    struct DexDataSet* typeIds;
    struct DexDataSet* fieldIds;
    struct DexDataSet* methodIds;
    struct DexDataSet* protoIds;
    struct DexDataSet* classDefs;
    struct DexDataItem* mapList;
    struct DexDataSet* typeListData; 
    struct DexDataSet* annoSetRefListData; 
    struct DexDataSet* annoSetData; 
    struct DexDataSet* classData; 
    struct DexDataSet* codeData;
    struct DexDataSet* stringData;
    struct DexDataSet* debugInfoData;
    struct DexDataSet* annoData;
    struct DexDataSet* encodeArrayData;
    struct DexDataSet* annosDirectoryData;
    struct MemMap memMap;
 
};


//void dexComputeSHA1Digest(const u1* data, u4 length, u1 digest[]);
void ReadClassDataHeader(const u1** pData, struct DexClassDataHeader *pHeader);
...
Bool verifyFields(struct DexFileData* dexData, u4 size, struct DexField* fields, Bool expectStatic);
Bool verifyMethods(struct DexFileData* dexData, u4 size, struct DexMethod* methods, Bool expectDirect);
...
//Int collectAnnoItemData(struct DexFile* dexFile, struct DexFileData* dexData, u4 refer_index, u4 refer_index1);
//Int collectDexAnnoSetItemData(struct DexFile* dexFile, struct DexFileData* dexData, Int option, u4 refer_index, u4 refer_index1);
//Int collectDexAnnoSetRefListData(struct DexFile* dexFile, struct DexFileData* dexData, u4 refer_annoDir, u4 refer_index1);
//Int collectDexAnnoDirectoryItemData(struct DexFile* dexFile, struct DexFileData* dexData, u4 clzDefIndex);
...
//Int collectDexDebugInfoData(struct DexFile* dexFile, struct DexFileData* dexData, u4 codeIndex);
...
//void writeClassAnnotations(struct DexFile* dexFile, struct DexFileData* dexData, struct DexClassDef* classDef, Int refer_annoDir);
...
//void* UnpackingDex(void* parament, const char* dumppath);
