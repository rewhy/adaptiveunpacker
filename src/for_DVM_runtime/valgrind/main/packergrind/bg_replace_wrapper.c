...
#include "bg_libdvm_wrapper.c"
#endif

#if TRACE_ART_PLATFORM
#include "bg_libart_wrapper.c"
#endif
...
long ptrace_wrapper(int request, int pid, int addr, int data) {
	...
	DO_CREQ_v_WWWW(VG_USERREQ__WRAPPER_PTRACE,	int, pid, int, addr, int, data, int, res);
	return res;
}
...
{ \
	...
	VALGRIND_CHECK_MEM_TAINTED("strcmp", s1_org, (Addr)s1-(Addr)s1_org); \					 
	VALGRIND_CHECK_MEM_TAINTED("strcmp", s2_org, (Addr)s2-(Addr)s2_org); \
...
#endif
#if 0 // Without tracing string comparing functions
/*---------------------- strrchr ----------------------*/
...
{ \
	...
	UWord nlen = 0, hlen = 0; \
	while (h[hlen]) hlen++; \
	VALGRIND_CHECK_MEM_TAINTED("strstr", h, hlen);\
	while (n[nlen]) nlen++;\
