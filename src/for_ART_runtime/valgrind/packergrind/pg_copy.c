...
IRStmt* mallocIRStmt_StoreG(IREndness end, IRExpr* addr, 
		IRExpr* data, IRExpr* guard)
{
	IRStmt* s            = VG_(malloc)("IRStmt", sizeof(IRStmt));
	IRStoreG* sg          = VG_(malloc)("IRStoreG", sizeof(IRStoreG));
	s->tag               = Ist_StoreG;
...
IRStmt* mallocIRStmt_LoadG(IREndness end, IRLoadGOp cvt, IRTemp dst,
		IRExpr* addr, IRExpr* alt, IRExpr* guard)
{
	IRStmt* s            = VG_(malloc)("IRStmt", sizeof(IRStmt));
	IRLoadG* lg          = VG_(malloc)("IRLoadG", sizeof(IRLoadG));
	s->tag               = Ist_LoadG;
...
IRStmt* deepMallocIRStmt(IRStmt* s)
{
	switch (s->tag) {
		...
		case Ist_StoreG:
			return mallocIRStmt_StoreG(s->Ist.StoreG.details->end, 
					deepMallocIRExpr(s->Ist.StoreG.details->addr),
					deepMallocIRExpr(s->Ist.StoreG.details->data),
					deepMallocIRExpr(s->Ist.StoreG.details->guard));
		case Ist_LoadG:
			return mallocIRStmt_LoadG(s->Ist.LoadG.details->end, s->Ist.LoadG.details->cvt, 
					s->Ist.LoadG.details->dst,
					deepMallocIRExpr(s->Ist.LoadG.details->addr),
					deepMallocIRExpr(s->Ist.LoadG.details->alt),
					deepMallocIRExpr(s->Ist.LoadG.details->guard));
		case Ist_Dirty: 
